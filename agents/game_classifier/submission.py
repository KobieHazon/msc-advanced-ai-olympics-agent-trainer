...
class BaseAgentModel(metaclass=ABCMeta):
    ...
    def load(self, actor_file_path: str):
        if os.path.exists(actor_file_path):
            actor = torch.load(actor_file_path, map_location=device, weights_only=True)
...
class CNN_Actor(nn.Module):
    def __init__(self, state_space, action_space, hidden_size=64):
        ...
        self.value_net = nn.Sequential(
            nn.Flatten(), nn.Linear(64 * 6 * 6, 128), nn.ReLU(), nn.Linear(128, action_space)
        )
...
class CNN_Critic(nn.Module):
    def __init__(self, state_space, hidden_size=64):
        ...
        self.value_net = nn.Sequential(
            nn.Flatten(), nn.Linear(64 * 6 * 6, 128), nn.ReLU(), nn.Linear(128, 1)
        )
...
class PPOAgentModel(BaseAgentModel):
    ...
    def update_result(self, is_win: bool):
        ...
        state = torch.tensor(np.array([t.state for t in self.transitions]), dtype=torch.float).to(
            device
        )
        action = (
...
)  # if you have a GPU, otherwise it will use CPU
game_classifier_model = GameClassifierNet().to(device)

criterion = nn.NLLLoss()
optimizer = torch.optim.Adam(game_classifier_model.parameters(), lr=0.01)

game_classifier_model_file_path = (
    os.path.dirname(os.path.abspath(__file__))
    + f"{os.sep}model-game-classifier-loss023-acc-0905.pth"
)
game_classifier_model.load_state_dict(
    torch.load(game_classifier_model_file_path, map_location=device, weights_only=True)
)
...
GAME_CLASSIFIER_ID_TO_AGENT[0].load(
    os.path.dirname(os.path.abspath(__file__)) + f"{os.sep}football_actor.pth"
)
GAME_CLASSIFIER_ID_TO_AGENT[4].load(
    os.path.dirname(os.path.abspath(__file__)) + f"{os.sep}running_actor.pth"
)
...
def my_controller(obs_list, action_space_list, obs_space_list):
    ...
    if state.shape != (40, 40):
        ...
        input_data = torch.as_tensor(state, dtype=torch.float32, device=device)
    else:
        ...
        with torch.no_grad():
            ...
            agent = GAME_CLASSIFIER_ID_TO_AGENT.get(predicted_game, RandomAgentModel())
    with torch.no_grad():
