...
class AgentTrainer:
    SAVE_DIR_BASE = "agent_trainer_saves"
    ...
    def __init__(self, agent_trainer_args: AgentTrainerArgs):
        ...
        self.game_env = self.game_factory.create_game_environment(agent_trainer_args.game_name)

        agent_model_factory = AgentModelFactory(SUPPORTED_AGENT_MODELS)
        self.training_agent_model = agent_model_factory.create_agent_model(
            agent_trainer_args.train_model
        )
        if agent_trainer_args.load_train_model:
            self.training_agent_model.load(agent_trainer_args.train_model_path)
        self.enemy_agent_model = agent_model_factory.create_agent_model(
            agent_trainer_args.enemy_model
        )
        ...
        torch.manual_seed(agent_trainer_args.torch_seed)
        self.save_dir = (
            f"{self.SAVE_DIR_BASE}{os.sep}"
            f"{agent_trainer_args.train_model}_vs_{agent_trainer_args.enemy_model}{os.sep}"
            f"{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
        )
        self.max_episodes = agent_trainer_args.max_episodes
        self.max_episode_len = agent_trainer_args.episode_max_len
        self.save_interval = agent_trainer_args.save_interval
        self.is_render_graphics = agent_trainer_args.render_graphic

        print(f"Arguments for AgentTrainer are:\n{agent_trainer_args}")
        logger.debug("Finished Initializing Agent Trainer")

    def _get_enemy_action(
        self, enemy_agent: BaseAgentModel, state: np.ndarray, random_prob: float = 0.2
    ) -> list[float]:
        """
        Randomize the enemy movement every few times
        """
        if random.random() < random_prob or isinstance(enemy_agent, RandomAgentModel):
            return self.RANDOM_SINGLETON.get_action(state)

        state_tensor = torch.as_tensor(state, dtype=torch.float32).unsqueeze(0)
        action_index, _ = enemy_agent.get_action(state_tensor)
        return actions_map[action_index]
    ...
    def run_episode(self, train_agent_index: int = 0):
        ...
        if isinstance(state[train_agent_index], type({})):
            train_agent_obs = state[train_agent_index]["agent_obs"]
            enemy_agent_obs = state[1 - train_agent_index]["agent_obs"]
        else:
            train_agent_obs = state[train_agent_index]
            enemy_agent_obs = state[1 - train_agent_index]

        train_agent_obs = torch.Tensor(
            np.array(train_agent_obs)
        )  # (40, 40) -> torch.Size([1, 40, 40])
        train_agent_obs = train_agent_obs.unsqueeze(0)
        reward = [0, 0]
        for _step_num in range(self.max_episode_len):
            enemy_action = self._get_enemy_action(self.enemy_agent_model, enemy_agent_obs)
            train_agent_action_index, action_prob = self.training_agent_model.get_action(
                train_agent_obs
            )
            train_agent_action = actions_map[train_agent_action_index]
            action = (
                [enemy_action, train_agent_action]
                if train_agent_index == 1
                else [train_agent_action, enemy_action]
            )

            next_state, reward, done, _ = self.game_env.step(action)

            if isinstance(next_state[train_agent_index], type({})):
                next_obs_ctrl_agent = next_state[train_agent_index]["agent_obs"]
                next_obs_oppo_agent = next_state[1 - train_agent_index]["agent_obs"]
            else:
                next_obs_ctrl_agent = next_state[train_agent_index]
                next_obs_oppo_agent = next_state[1 - train_agent_index]

            next_obs_ctrl_agent = torch.Tensor(
                np.array(next_obs_ctrl_agent)
            )  # (40, 40) -> torch.Size([1, 40, 40])
            ...
            enemy_agent_obs = next_obs_oppo_agent
            train_agent_obs = next_obs_ctrl_agent
    ...
    def train(self):
        ...
        for episode_num in range(self.max_episodes):
            ...
            print(
                ...
                episode_num,
                "controlled agent: ",
                train_agent_index,  # "; Episode Return: ", Gt,
                "; win rate(controlled & opponent): ",
                "%.2f" % (sum(train_win_record) / len(train_win_record)),
                "%.2f" % (sum(enemy_win_record) / len(enemy_win_record)),
                "; Trained episode:",
                train_count,
            )
            if is_train_win:
