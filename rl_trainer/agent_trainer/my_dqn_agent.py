# DQN Agent - 1 game at a time.
# Reward function (winning oriented):
# 1000 - Steps + IfWon Max(1000 - steps//5 + 10000, 0)
...
from olympics_engine.scenario import table_hockey, football, Running_competition, billiard
...
class DQNAgent:
    ...
    def __init__(self, state_space, action_space, max_memory_size, batch_size, gamma, lr,
        ...
        # DQN network
        self.dqn = DQN_Net(state_space, action_space, dropout).to(self.device)
...
def run(training_mode, pretrained, num_episodes=100000, exploration_max=1):
    num_agents = 2
    ...
    for ep_num, env in zip(tqdm(range(num_episodes)), training_games()):
        state = env.reset()
        steps = 0
        ...
        if ep_num != 0 and ep_num % 100 == 0:
            ...
            if training_mode:
                episode_dir = f"./{datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}/episode_{ep_num}"
