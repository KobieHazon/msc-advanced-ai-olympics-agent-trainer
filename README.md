...
A 2023 CS MSc Advanced AI team project for training and submitting agents to the IJCAI AI Olympics integrated environment. The work combines a reusable PPO trainer, DQN and dueling-DQN experiments, a visual game classifier, game-specific policy dispatch, and final trained submission weights.

| Material | Classification | Notes |
| --- | --- | --- |
| `env/`, `olympics_engine/`, `utils/`, `agents/random/`, assets, and original documentation | Supplied framework | The `jidiai/Competition_IJCAI2023` environment and support code. |
| `agents/dueling_dqn/` and `agents/game_classifier/` | Coauthored solution | Final submission agents and trained weights from the team snapshot. |
...
- Final classifier, football actor, and running actor weights used by the submission
- The supplied AI Olympics engine, wrappers, assets, and reference trainers needed to run the project
- One curated latest PPO actor/critic checkpoint from the 9,900-episode run
...
Training is stochastic and expensive. The command demonstrates the canonical entry point; it does not reproduce the original long run in a few minutes.
...
The tests load the model weights, exercise both final submission controllers, validate the trainer factories and argument handling, and smoke-test an environment reset and step without rendering a window.
